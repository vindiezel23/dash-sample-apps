...
zone_index_to_borough_index = {
    int(index): borough_name_to_index[zbmapper[name]] for index, name in zmapper.items()
}
...
df_original.categorize(
    df_original.pickup_day,
...
def create_figure_heatmap(data_array, heatmap_limits, trip_start, trip_end):
    ...
    if trip_start:
        add_point(
            trip_start[0], trip_start[1], name="Trip start", marker_symbol="circle"
        )
...
def create_figure_geomap(
    pickup_counts, zone, zoom=10, center={"lat": 40.7, "lon": -73.99}
):
    geomap_data = {
        "count": pickup_counts,
    ...
    geo_json_selected["features"] = [
        feature
        for feature in geo_json_selected["features"]
        if feature["properties"]["zone"] == zone_index_to_name[zone]
    ]

    geomap_data_selected = {
        "zone_name": [geo_json_selected["features"][0]["properties"]["zone"],],
        "default_value": ["start"],
...
def create_figure_sankey(
    df_outflow_top, df_outflow_rest, df_outflow_borough, pickup_zone
):
    ...
    labels = (
        list(bmapper.values())
        + list(zmapper.values())
        + list([f"Other: {k}" for k in bmapper.values()])
    )

    # overwrite the borough label with one that includes the zone name
    start_name = (
        f"{borough_index_to_name[pickup_borough]} - {zone_index_to_name[pickup_zone]}"
    )
    labels[pickup_borough] = start_name

    source = (
        df_outflow_borough.pickup_borough.astype("int").tolist()
        + df_outflow_top.dropoff_borough.astype("int").tolist()
        + df_outflow_rest.dropoff_borough.astype("int").tolist()
    )

    zone_indices = (
        df_outflow_top.dropoff_zone.astype("int") + label_offset_zones
    ).tolist()
    zone_indices_other = (
        df_outflow_rest.dropoff_zone.astype("int") + label_offset_zones
    ).tolist()
    target = (
        df_outflow_borough.dropoff_borough.astype("int").tolist()
        + zone_indices
        + zone_indices_other
    )
...
def create_figure_sunburst(
    ...
    labels = (
        [start_name]
        + df_outflow_borough.dropoff_borough_name.tolist()
        + df_outflow_top.dropoff_zone_name.tolist()
    )
    parents = (
        [""]
        + [start_name] * len(df_outflow_borough)
        + df_outflow_top.dropoff_borough_name.tolist()
    )
    counts = np.array(
        [len(df)]
        + df_outflow_borough.count_trips.tolist()
...
def create_table_data(df_outflow_top):
    ...
    df["borough"] = df.dropoff_zone.map(zone_index_to_borough_index).map(
        borough_index_to_name
    )
    records = []
...
def compute_heatmap_data(days, hours, heatmap_limits):
    logger.info(
        "Compute: heatmap data: days=%r hours=%r limits=%r", days, hours, heatmap_limits
    )
    df, selection = create_selection(days, hours)
    heatmap_data_array = df.count(
        binby=[df.pickup_longitude, df.pickup_latitude],
        selection=selection,
...
def compute_trip_details(days, hours, trip_start, trip_end):
    logger.info(
        "Compute: trip details: days=%r hours=%r trip_start=%r trip_end=%r",
        days,
        hours,
        trip_start,
        trip_end,
    ...
    selection_pickup = (df.pickup_longitude - pickup_long) ** 2 + (
        df.pickup_latitude - pickup_lat
    ) ** 2 <= r ** 2
    selection_dropoff = (df.dropoff_longitude - dropoff_long) ** 2 + (
    ...
    return {
        ...
        "counts_total": df.count(
            binby=[df.total_amount],
            limits=[limits_amount],
            shape=bins,
            selection=selection,
        ),
        "counts_duration": df.count(
            binby=[df.trip_duration_min],
            limits=[limits_duration],
            shape=bins,
            selection=selection,
        ),
    }
...
def compute_flow_data(days, hours, zone):
    ...
    if df_outflows_rest:
        ...
        df_outflow_rest = vaex.from_scalars(
            pickup_borough=-1, dropoff_borough=-1, dropoff_zone=-1, count_trips=-1
        )[:0]
...
trip_detail_data_initial = compute_trip_details(
    [], [0, 23], trip_start_initial, trip_end_initial
)
...
figure_sankey_initial = create_figure_sankey(
    df_outflow_top_initial,
    df_outflow_rest_initial,
    df_outflow_borough_initial,
    zone_initial,
)
figure_sunburst_initial = create_figure_sunburst(
    df_outflow_top_initial,
    df_outflow_rest_initial,
    df_outflow_borough_initial,
    zone_initial,
...
zone_summary_md = zone_summary_template_md.format(
    zone_index_to_name[zone_initial], zone_pickup_count, len(df_original)
)
...
app.layout = html.Div(
    ...
    children=[
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.H4(
                                    f"Exploring {len(df_original)//1_000_000:} Million Taxi trips in Real Time"
                                ),
                            ],
                        ),
                        html.Div(
                ...
                html.Div(
                    ...
                    children=[
                        dcc.Loading(
                            ...
                            children=[
                                html.Div(
                                    id="loader-trigger-1", style={"display": "none"}
                                ),
                                html.Div(
                                    id="loader-trigger-2", style={"display": "none"}
                                ),
                                html.Div(
                                    id="loader-trigger-3", style={"display": "none"}
                                ),
                                html.Div(
                                    id="loader-trigger-4", style={"display": "none"}
                                ),
                                dcc.Markdown(
        ...
        dcc.Tabs(
            ...
            children=[
                dcc.Tab(
                    label="Popular destinations in New York",
                    children=[
                        html.Div(
                            className="row",
                            children=[
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Markdown(
                                            id="zone_summary", children=zone_summary_md
                                        ),
                                        dcc.Graph(
                                            id="geomap_figure",
                                            figure=create_figure_geomap(
                                                geomap_data_initial, zone_initial
                                            ),
                                            config={
                        ...
                        html.Div(
                            ...
                            children=[
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Markdown(
                                            children="_Click on the map to select trip start and destination._"
                                        ),
                                        dcc.Graph(
                                            id="heatmap_figure",
                                            figure=create_figure_heatmap(
                                                heatmap_data_initial,
...
# Map figure
@app.callback(
    [Output("heatmap_figure", "figure"), Output("loader-trigger-4", "children")],
    [
        Input("days", "value"),
        Input("hours", "value"),
...
def update_heatmap_figure(days, hours, heatmap_limits, trip_start, trip_end):
    ...
    return (
        create_figure_heatmap(data_array, heatmap_limits, trip_start, trip_end),
        "trigger loader",
    )
...
def update_limits(relayoutData, heatmap_limits):
    logger.info(
        "Interaction: map/zoom on heatmap detected: relayoutData=%r heatmap_limits=%r",
        relayoutData,
        heatmap_limits,
    )
    if relayoutData is None:
        ...
        d = relayoutData
        heatmap_limits = [
            [d["xaxis.range[0]"], d["xaxis.range[1]"]],
            [d["yaxis.range[0]"], d["yaxis.range[1]"]],
        ]
    else:
        raise dash.exceptions.PreventUpdate
...
def click_heatmap_action(click_data_heatmap, map_clicks, trip_start, trip_end):
    ...
    if click_data_heatmap is not None:
        point = (
            click_data_heatmap["points"][0]["x"],
            click_data_heatmap["points"][0]["y"],
        )
...
@app.callback(
    ...
    [State("zone", "data"), State("table", "data")],
    prevent_initial_call=True,
)
def click_action(
    click_data_geomap,
...
@app.callback(
    ...
    [Input("days", "value"), Input("hours", "value"), Input("zone", "data")],
    [State("geomap_figure", "figure")],
    prevent_initial_call=True,
)
def update_geomap_figure(days, hours, zone, current_figure):
    logger.info(
        "Figure: updating geo map for: days=%r hours=%r zone=%r", days, hours, zone
    )
    ...
    zone_summary_md = zone_summary_template_md.format(
        zone_index_to_name[zone], zone_pickup_count, len(df_original)
    )
...
def update_flow_figures(days, hours, zone):
    ...
    pickup_zone = zone
    fig_sankey = create_figure_sankey(
        df_outflow_top, df_outflow_rest, df_outflow_borough, pickup_zone
    )
    fig_sunburst = create_figure_sunburst(
        df_outflow_top, df_outflow_rest, df_outflow_borough, pickup_zone
    )
...
@app.callback(
    [
        Output("trip_summary_amount_figure", "figure"),
...
def trip_details_summary(days, hours, trip_start, trip_end):
    ...
    fig_amount, peak_amount = create_figure_histogram(
        df_original.bin_edges(df_original.total_amount, limits_amount, shape=bins),
        counts_total,
        title=None,
        xlabel="Total amount [$]",
    ...
    # The trip duration
    fig_duration, peak_duration = create_figure_histogram(
        df_original.bin_edges(df_original.trip_duration_min, limits_amount, shape=bins),
        counts_duration,
        title=None,
